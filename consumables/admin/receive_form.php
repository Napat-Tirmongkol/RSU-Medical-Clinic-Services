<?php
// consumables/admin/receive_form.php — รับเข้าสต็อก
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($consumableId <= 0) $errors[] = 'กรุณาเลือกวัสดุ';
    ...
    if (empty($errors) && $consumable) {
        ...
        try {
            ...
            csm_log_txn(
                $pdo, $consumableId, 'receive',
                +$qtyPieces, $unitInput, $qtyInput,
                null, null, 'รับเข้าสต็อก', $reference ?: null, $note ?: null, $txnDate
            );
            $pdo->commit();
            $success = true;
...
<?php if ($success): ?>
    <div class="mb-4 p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm">
        <i class="fas fa-circle-check"></i> บันทึกการรับเข้าเรียบร้อย
    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div class="md:col-span-2">
                ...
                <input type="number" name="qty_input" id="qty_input" min="1" required class="asset-input" placeholder="เช่น 2">
            </div>
            <div>
                <label class="asset-label">หน่วย</label>
                <select name="unit_input" id="unit_input" class="asset-input" onchange="csmRecalcQty()">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
                <label class="asset-label">วันที่รับเข้า</label>
                <input type="date" name="txn_date" value="<?= htmlspecialchars(date('Y-m-d')) ?>" class="asset-input">
            </div>
            <div>
                <label class="asset-label">เลขที่เอกสาร / PO</label>
                <input type="text" name="reference" class="asset-input" placeholder="เช่น PO-2026-001">
            </div>
        </div>

        <div>
            <label class="asset-label">หมายเหตุ</label>
            <textarea name="note" rows="2" class="asset-input" placeholder="เช่น รับจาก vendor X / ลอตวันที่ ..."></textarea>
        </div>
    </div>
...
function csmUpdateStockHint() {
    ...
    if (!opt || !opt.value) { hint.textContent = ''; document.getElementById('qty_calc_hint').textContent = ''; return; }
    ...
    hint.innerHTML = '<i class="fa-solid fa-circle-info"></i> คงเหลือปัจจุบัน <strong>' + stock + '</strong> ' + unitPiece +
