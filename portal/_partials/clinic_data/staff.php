                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <?php else: foreach ($rows as $r): ?>
                        <tr class="hover:bg-slate-50/60" id="staff-row-<?= (int)$r['id'] ?>"
                            ...
                            data-full-name="<?= htmlspecialchars((string)$r['full_name'], ENT_QUOTES) ?>"
                            data-license-no="<?= htmlspecialchars((string)($r['license_no'] ?? ''), ENT_QUOTES) ?>"
                            data-role="<?= htmlspecialchars((string)$r['role'], ENT_QUOTES) ?>"
                            data-department="<?= htmlspecialchars((string)($r['department'] ?? ''), ENT_QUOTES) ?>"
                            ...
                            <td class="px-5 py-3 text-right whitespace-nowrap">
                                <button onclick="msEdit(<?= (int)$r['id'] ?>)"
                                    title="แก้ไข"
...
async function msEdit(id) {
    ...
    const cur = {
        ...
        license_no: row.dataset.licenseNo || '',
        role:       row.dataset.role || 'doctor',
        department: row.dataset.department || '',
        phone:      row.dataset.phone || '',
        email:      row.dataset.email || '',
    };

    const titles = ['นพ.','พญ.','ทพ.','ทญ.','ภก.','ภญ.','พย.','คุณ'];
    const roles  = {doctor:'แพทย์', nurse:'พยาบาล', pharmacist:'เภสัชกร', dentist:'ทันตแพทย์', other:'อื่นๆ'};

    const esc = s => String(s ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
    const titleOpts = '<option value="">— คำนำหน้า —</option>' +
        titles.map(t => `<option value="${esc(t)}" ${cur.title === t ? 'selected' : ''}>${esc(t)}</option>`).join('');
    const roleOpts = Object.entries(roles)
        .map(([k,v]) => `<option value="${esc(k)}" ${cur.role === k ? 'selected' : ''}>${esc(v)}</option>`).join('');

    const inputCls = 'px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-800 outline-none focus:border-blue-400 w-full';

    const result = await Swal.fire({
        title: 'แก้ไขข้อมูลบุคลากร',
        width: 640,
        html: `
            <div style="text-align:left; display:grid; grid-template-columns:1fr 2fr 1fr; gap:.6rem; margin-bottom:.6rem;">
                <select id="msE-title" class="${inputCls}">${titleOpts}</select>
                <input id="msE-full_name" type="text" placeholder="ชื่อ-นามสกุล *" value="${esc(cur.full_name)}" class="${inputCls}">
                <select id="msE-role" class="${inputCls}">${roleOpts}</select>
            </div>
            <div style="text-align:left; display:grid; grid-template-columns:1fr 1fr; gap:.6rem;">
                <input id="msE-license_no" type="text" placeholder="เลขใบอนุญาต" value="${esc(cur.license_no)}" class="${inputCls}">
                <input id="msE-department" type="text" placeholder="แผนก/หน่วยงาน" value="${esc(cur.department)}" class="${inputCls}">
                <input id="msE-phone" type="tel" placeholder="เบอร์โทร" value="${esc(cur.phone)}" class="${inputCls}">
                <input id="msE-email" type="email" placeholder="อีเมล" value="${esc(cur.email)}" class="${inputCls}">
            </div>
        `,
        ...
        focusConfirm: false,
        preConfirm: () => {
            const v = id => document.getElementById('msE-' + id).value.trim();
    ...
    const res = await msPost('edit', { id, ...result.value });
    if (res.ok) {
        showPortalToast(res.message || 'อัปเดตแล้ว', 'success');
        setTimeout(() => cdReload('staff'), 600);
    } else {
