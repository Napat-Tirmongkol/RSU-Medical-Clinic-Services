...
switch ($action) {
    case 'quota':
        ...
        echo json_encode(['status' => 'ok', 'quota' => $quota, 'consumption' => $consumption]);
        break;

    case 'delivery':
        $date = preg_replace('/[^0-9]/', '', $_GET['date'] ?? date('Ymd', strtotime('-1 day')));
        if (strlen($date) !== 8) {
        ...
        echo json_encode(['status' => 'ok', 'date' => $date, 'data' => $data]);
        break;
