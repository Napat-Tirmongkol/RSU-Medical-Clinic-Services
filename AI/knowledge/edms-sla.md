...
| `sys_clinic_hours` (existing) | เวลาทำการ + วันหยุด — **shared กับโมดูลอื่น** (clinic_status, doctor_schedule). SLA อ่านอย่างเดียวผ่าน `get_clinic_hours_for_date()`. ตั้งค่าที่ `?section=clinic_data&cd_view=hours` |
...
- ถ้า `$businessOnly = true` → เดิน business time ทีละช่วง จนสะสมครบ:
  - ถ้าวันนั้น `closed=true` (วันหยุด / ไม่มี regular) → กระโดดไปต้นวันถัดไป
  - ถ้าก่อนเวลาเปิด → fast-forward ไป open_time
...
### Calendar source — **อ้างอิงจาก `sys_clinic_hours`** (ปฏิทินคลินิก)
```php
$hours = sla_get_day_hours($pdo, '2026-05-23');
// wraps get_clinic_hours_for_date() ใน clinic_status_helper.php
// returns ['closed' => bool, 'start' => 'HH:MM:SS'|null, 'end' => 'HH:MM:SS'|null]
```

Logic ของ `get_clinic_hours_for_date()`:
1. หา type IN ('holiday','special') ที่ specific_date ตรง — special override holiday
2. ถ้าไม่มี → หา type='regular' ที่ weekday ตรง
3. ถ้าไม่มีเลย → คืน closed=true

### Fallback safety
ถ้า `sys_clinic_hours` ว่างเปล่า → ทุกวันเป็น closed → SLA จะ extend deadline ออกไปเรื่อยๆ จน hit hard cap (365 วัน). **ก่อน enable SLA module ต้องตั้งค่าปฏิทินคลินิกก่อน** (ตั้งที่ `?section=clinic_data&cd_view=hours`)
...
### Calendar admin — **ไม่มีของตัวเอง** ใช้ปฏิทินคลินิก
- ตั้งค่าที่ `?section=clinic_data&cd_view=hours` (`portal/_partials/clinic_data/hours.php`)
- รองรับ 3 type: `regular` (รายสัปดาห์) / `holiday` (วันหยุด) / `special` (เปิดพิเศษ)
- การเปลี่ยนแปลงปฏิทินคลินิก = กระทบเวลา SLA ของเอกสารใหม่ทันที (เอกสารเก่าไม่ recompute deadline)
- Link จากหน้า policies ไปยังปฏิทินคลินิกผ่าน hyperlink "ปฏิทินคลินิก"
...
| (calendar) | — | ไม่มี — ใช้ปฏิทินคลินิก `?section=clinic_data&cd_view=hours` |
...
portal/ajax_edms_sla.php                   ← AJAX endpoints (13 actions)
portal/_partials/edms/sla_dashboard.php    ← KPI + charts
portal/_partials/edms/sla_policies.php     ← policy matrix admin
portal/_partials/clinic_data/hours.php     ← **ใช้ตัวนี้สำหรับเวลาทำการ + วันหยุด** (existing, ไม่ต้องสร้างใหม่)
...
- [ ] **ตั้งค่าปฏิทินคลินิกก่อน** (`?section=clinic_data&cd_view=hours`) — ใส่เวลา regular Mon-Fri + วันหยุดราชการ. ถ้าไม่ตั้ง SLA จะ extend deadline ออกไปจน hard cap
