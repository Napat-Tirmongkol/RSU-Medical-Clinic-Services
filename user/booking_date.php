...
<div class="p-5 pb-32 flex flex-col h-full bg-[#e2f4ea]">
    <div class="mb-6">
    ...
    <div class="bg-white rounded-3xl p-5 shadow-sm border border-gray-100">
        <div class="flex items-center justify-between mb-6">
            <a href="?year=<?= $prevYear ?>&month=<?= $prevMonth ?>&campaign_id=<?= $campaignId ?>" class="w-10 h-10 flex items-center justify-center rounded-xl bg-gray-50 text-gray-600 hover:bg-[#2e9e63] hover:text-white transition-colors">
                <i class="fa-solid fa-chevron-left"></i>
            </a>
            <h3 class="text-lg font-bold text-gray-900 font-prompt"><?= date('F Y', mktime(0,0,0,$month,1,$year)) ?></h3>
            <a href="?year=<?= $nextYear ?>&month=<?= $nextMonth ?>&campaign_id=<?= $campaignId ?>" class="w-10 h-10 flex items-center justify-center rounded-xl bg-gray-50 text-gray-600 hover:bg-[#2e9e63] hover:text-white transition-colors">
        ...
        <div class="grid grid-cols-7 gap-y-4 text-center">
            ...
            for ($d = 1; $d <= $daysInMonth; $d++) {
                ...
                if ($selectedDate == $d && $density !== 'none' && !$isPast) {
                    $btnClass = 'bg-[#2e9e63] text-white shadow-md shadow-blue-200';
                }
...
<div class="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-4 bg-white border-t border-gray-100 z-20 flex gap-3 shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.1)]">
    ...
    <a id="btn-next"
        ...
        class="flex-1 bg-[#2e9e63] hover:bg-green-700 text-white font-bold py-4 rounded-xl transition-all shadow-sm text-center <?= ($selectedDate === null || density_for_day($year, $month, (int)$selectedDate, $dailyStats, $dbError) === 'high' || strtotime(sprintf("%04d-%02d-%02d", $year, $month, $selectedDate)) < strtotime(date('Y-m-d'))) ? 'hidden' : '' ?>">ดำเนินการต่อ</a>
</div>
...
function selectDay(day, density) {
    ...
    document.querySelectorAll('[data-day]').forEach(btn => {
        btn.classList.remove('bg-[#2e9e63]', 'text-white', 'shadow-md', 'shadow-blue-200');
        btn.classList.add('text-gray-700', 'hover:bg-gray-50');
    });
    const target = document.querySelector('[data-day="' + day + '"]');
    if (target) {
        target.classList.remove('text-gray-700', 'hover:bg-gray-50');
        target.classList.add('bg-[#2e9e63]', 'text-white', 'shadow-md', 'shadow-blue-200');
    }
