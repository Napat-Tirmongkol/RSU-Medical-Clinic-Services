...
<body class="text-slate-900 pb-32">
    ...
    <div class="max-w-md mx-auto relative min-h-screen">
        ...
        <!-- ── Premium Header ── -->
        <header class="glass-header sticky top-0 z-[60] px-6 py-6 flex items-center justify-between shadow-[0_10px_30px_rgba(0,82,204,0.15)] rounded-b-[2.5rem] border-b border-white/10">
            <div class="flex items-center gap-4">
                <div onclick="showProfile()" class="w-11 h-11 rounded-2xl bg-white/20 p-0.5 border border-white/30 shadow-inner active:scale-95 transition-all cursor-pointer overflow-hidden">
                    <img src="<?= $user['picture_url'] ?? 'https://ui-avatars.com/api/?name='.urlencode($user['full_name']); ?>" class="w-full h-full object-cover rounded-[0.9rem]">
                </div>
                <div>
                    <p class="text-blue-100 text-[9px] font-black uppercase tracking-[0.2em] leading-none mb-1.5 opacity-80"><?= $greeting ?></p>
                    <h1 class="text-white font-black text-base leading-none tracking-tight"><?= $user['full_name'] ?></h1>
                </div>
            </div>
            <div class="flex items-center gap-2.5">
                <button onclick="showNotifications()" class="w-10 h-10 flex items-center justify-center bg-white/10 rounded-xl border border-white/10 active:bg-white/20 transition-all relative">
                    ...
                    <?php if ($upcoming_count > 0): ?>
                        <span class="absolute top-2.5 right-2.5 w-2 h-2 bg-red-500 rounded-full border-2 border-[#004bb8]"></span>
                    <?php endif; ?>
                </button>
                <button onclick="showQR()" class="w-10 h-10 flex items-center justify-center bg-white/10 rounded-xl border border-white/10 active:bg-white/20 transition-all">
                    <i class="fa-solid fa-grid-view text-white text-sm"></i>
                </button>
            </div>
        </header>
