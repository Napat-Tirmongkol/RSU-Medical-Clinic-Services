            ...
            <?php if (defined('SITE_SHOW_INSURANCE') && SITE_SHOW_INSURANCE): ?>
                <div class="space-y-4">
                    ...
                    <?php else:
                        ...
                        $coverStartText = $coverStart ? date('d/m/Y', strtotime($coverStart)) : '—';
                        $coverEndText   = $coverEnd   ? date('d/m/Y', strtotime($coverEnd))   : '—';
                    ...
                    <div class="bg-slate-900 rounded-[3rem] p-8 shadow-2xl relative overflow-hidden premium-shadow">
                        ...
                        <div class="flex items-start justify-between mb-7 relative z-10">
                            <div>
                        ...
                        <div class="mb-5 relative z-10">
                            <p class="text-white/30 text-[9px] font-black uppercase tracking-[0.2em] mb-1">เลขที่กรมธรรม์ · Policy No.</p>
                            <p class="text-white text-sm font-black tracking-widest"><?= htmlspecialchars($policyNo) ?></p>
                        </div>
                        <?php endif; ?>

                        <!-- Policyholder -->
                        <div class="mb-5 pt-5 border-t border-white/10 relative z-10">
                            <p class="text-white/30 text-[9px] font-black uppercase tracking-[0.2em] mb-1">ผู้เอาประกันภัย · Policyholder</p>
                            <p class="text-white text-[12px] font-black uppercase tracking-wider truncate"><?= htmlspecialchars($user['full_name']) ?></p>
                        </div>

                        <!-- Coverage Period -->
                        <div class="mb-6 pt-5 border-t border-white/10 relative z-10">
                            <p class="text-white/30 text-[9px] font-black uppercase tracking-[0.2em] mb-2.5">ระยะเวลาประกันภัย · Coverage Period</p>
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <p class="text-white/40 text-[8px] font-black uppercase tracking-[0.15em] mb-1">วันที่เริ่มใช้</p>
                                    <p class="text-white text-[11px] font-bold tracking-wider"><?= htmlspecialchars($coverStartText) ?></p>
                                </div>
                                <div>
                                    <p class="text-white/40 text-[8px] font-black uppercase tracking-[0.15em] mb-1">วันที่สิ้นสุด</p>
                                    <p class="text-[11px] font-bold tracking-wider <?= !$isActive ? 'text-rose-400' : 'text-white' ?>"><?= htmlspecialchars($coverEndText) ?></p>
                                </div>
                            </div>
                        </div>

                        <!-- Medical Coverage Amount -->
                        <div class="mt-6 -mx-8 px-8 py-5 bg-gradient-to-r from-emerald-500/15 via-emerald-500/10 to-transparent border-t border-emerald-400/20 relative z-10">
                            <p class="text-emerald-300/70 text-[9px] font-black uppercase tracking-[0.2em] mb-1.5">
                                วงเงินค่ารักษาพยาบาล <span class="text-emerald-400/60">(ต่ออุบัติเหตุ)</span>
                            </p>
                            <p class="text-white font-black tracking-tight">
                                <span class="text-3xl"><?= $medicalLimit ?></span>
                                <span class="text-sm text-white/60 font-bold ml-1">บาท</span>
                            </p>
                        </div>
                    </div>
                    <?php endif; ?>
