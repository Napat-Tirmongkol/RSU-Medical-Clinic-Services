...
### Phase ที่จบล่าสุด
- **Deploy folder rename `e-campaignv2` → `rsu-clinic` (2026-05-22)** — เปลี่ยนชื่อ deployment folder ให้ตรงกับ scope จริงของระบบ (RSU Medical Clinic Services) เพราะชื่อ `e-campaignv2` เป็น legacy จากตอนที่ระบบเป็นแค่ e-Campaign · แก้ 51 references ใน 32 ไฟล์: `.htaccess` (7 ErrorDocument) · `web.config` (rewrite rules) · `database/migrations/setup_links.php` + `portal/apps.php` (3 sys_app_links URLs ละไฟล์) · cron scripts (`backup_db.sh` + 6 PHP cron docs) · `admin/ajax/ajax_force_cancel.php` + `ajax_bulk_cancel_bookings.php` (LIFF fallback URLs) · `cron/daily_report_to_line.php` (report URL) · `user/c.php` (LINE login redirect) · `error_docs/not_found.html` (404 link + ajax endpoint) · docs/manuals · config comments. Rename file `e-campaignv2.code-workspace` → `rsu-clinic.code-workspace`. **Production deployment checklist** (ต้องทำเรียง):  1) backup DB ก่อน 2) rename `httpdocs/e-campaignv2` → `httpdocs/rsu-clinic` 3) แก้ `sys_app_links` table — `UPDATE sys_app_links SET url = REPLACE(url,'/e-campaignv2/','/rsu-clinic/') WHERE url LIKE '%/e-campaignv2/%'` 4) update Plesk virtual directory / IIS application name ถ้ามี 5) update cron tab — แก้ path จาก `/var/www/html/e-campaignv2/` เป็น `/var/www/html/rsu-clinic/` 6) แจ้ง LINE OA admin ถ้ามี hardcoded URL ใน rich menu / webhook config 7) smoke-test: portal login + error pages (`/rsu-clinic/errors/404.php`) + LINE LIFF redirect + cron backup
