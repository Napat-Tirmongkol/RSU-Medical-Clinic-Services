...
### Phase ที่จบล่าสุด
- **AI restoration Phases A + B (2026-05-18/19)** — บูรณะ AI auto-reply ที่ส่งคำตอบ stale ("พรุ่งนี้ที่ 17 พ.ค." ค้าง). **Phase A**: detect time-sensitive *question* → bypass FAQ cache → generate fresh (commit `5752680`). **Phase B**: per-FAQ `is_time_sensitive` flag + admin UI + auto-default on time-sensitive intent (commit `1166275`). Stale scanner UI + signed receipt URL (Phase 0/0.5) ก็ landed ในช่วงเดียวกัน. **Phase C ยังไม่เริ่ม** — answer cache + fallback chain + telemetry
