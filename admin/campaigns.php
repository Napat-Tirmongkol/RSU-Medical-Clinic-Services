...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add' && $title && $capacity >= 0) {
        try {
            ...
            $sql = "INSERT INTO camp_list (title, type, description, total_capacity, available_until, status, is_auto_approve, share_token, vaccine_type_id, room_id)
                    VALUES (:title, :type, :description, :capacity, :until, :status, :auto_approve, :token, :vtid, :room_id)";
