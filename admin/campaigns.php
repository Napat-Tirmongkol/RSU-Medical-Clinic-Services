...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // 3b. Quick campaign intake controls
    if (in_array($action, ['close_registration', 'mark_full', 'close_future_slots'], true)) {
        $id = (int) ($_POST['campaign_id'] ?? 0);
        if ($id > 0) {
            try {
                if ($action === 'close_registration') {
                    $stmt = $pdo->prepare("
                        UPDATE camp_list
                        SET available_until = DATE_SUB(CURDATE(), INTERVAL 1 DAY)
                        WHERE id = :id
                    ");
                    $stmt->execute([':id' => $id]);
                    $message = "ปิดช่วงรับสมัครเรียบร้อยแล้ว โดยยังคงข้อมูลไว้สำหรับสแกนหน้างาน";
                    $messageType = "success";
                    log_activity('close_registration', "ปิดช่วงรับสมัครแคมเปญ ID: {$id}");
                }

                if ($action === 'mark_full') {
                    $stmt = $pdo->prepare("UPDATE camp_list SET status = 'full' WHERE id = :id");
                    $stmt->execute([':id' => $id]);
                    $message = "ปรับสถานะแคมเปญเป็นเต็มแล้วเรียบร้อย";
                    $messageType = "success";
                    log_activity('mark_campaign_full', "ปรับแคมเปญ ID: {$id} เป็นเต็มแล้ว");
                }

                if ($action === 'close_future_slots') {
                    ...
                    $usedCapacity = (int) $usedStmt->fetchColumn();
                    ...
                    $capStmt = $pdo->prepare("
                        UPDATE camp_list
                        SET total_capacity = :used_capacity
                        WHERE id = :id
                    ...
                    $message = $slotCount > 0
                        ? "ปิดรอบอนาคตทั้งหมดเรียบร้อยแล้ว โดยไม่ยกเลิกผู้ที่จองไว้"
                        : "ไม่พบรอบอนาคตให้ปิด แต่ระบบอัปเดตโควต้ารวมตามยอดจองแล้ว";
                    $messageType = "success";
                    log_activity('close_future_slots', "ปิดรอบอนาคตของแคมเปญ ID: {$id} ({$slotCount} รอบ)");
                }
            } catch (PDOException $e) {
...
<style>
    ...
    .act-btn-close:hover { background: #475569; color: #fff; border-color: #475569; box-shadow: 0 4px 12px rgba(71,85,105,.25); transform: translateY(-1px); }
    .act-btn-full {
        background: #fff1f2; color: #e11d48; border-color: #fecdd3;
    }
    .act-btn-full:hover { background: #e11d48; color: #fff; border-color: #e11d48; box-shadow: 0 4px 12px rgba(225,29,72,.25); transform: translateY(-1px); }
    .act-btn-slots {
        background: #eff6ff; color: #2563eb; border-color: #bfdbfe;
    }
    .act-btn-slots:hover { background: #2563eb; color: #fff; border-color: #2563eb; box-shadow: 0 4px 12px rgba(37,99,235,.25); transform: translateY(-1px); }
    .act-btn-disabled {
                    ...
                    <tr class="glass-tr group campaign-row <?= $isInactive ? 'is-inactive' : 'is-active' ?>" style="<?= $isInactive ? 'opacity:.55' : '' ?>">
                        ...
                        <td class="px-3 py-4 text-center sticky right-0 bg-white group-hover:bg-[#f0fdf4] z-10 transition-colors" style="box-shadow:-8px 0 20px rgba(0,0,0,0.03); border-left:1px solid #f0f4fa">
                            <div class="grid grid-cols-3 gap-2 w-max mx-auto">
                                ...
                                <?php if (!$isExpired): ?>
                                <form method="POST" class="m-0"
                                    onsubmit="return confirm('ปิดช่วงรับสมัครของแคมเปญนี้? ผู้ที่จองไว้ยังสแกนเข้างานได้');">
                                    <?php csrf_field(); ?>
                                    <input type="hidden" name="action" value="close_registration">
                                    <input type="hidden" name="campaign_id" value="<?= $c['id'] ?>">
                                    <button type="submit" class="act-btn act-btn-close" title="หมดช่วงรับสมัคร">
                                        <i class="fa-solid fa-calendar-xmark text-xs"></i>
                                    </button>
                                </form>
                                <?php else: ?>
                                    <button type="button" class="act-btn act-btn-disabled" title="หมดช่วงรับสมัครแล้ว">
                                        <i class="fa-solid fa-calendar-xmark text-xs"></i>
                                    </button>
                                <?php endif; ?>
                                ...
                                <form method="POST" class="m-0"
                                    onsubmit="return confirm('ปรับแคมเปญนี้เป็นสถานะเต็มแล้ว?');">
                                    <?php csrf_field(); ?>
                                    <input type="hidden" name="action" value="mark_full">
                                    <input type="hidden" name="campaign_id" value="<?= $c['id'] ?>">
                                    <button type="submit" class="act-btn act-btn-full" title="เต็มแล้ว">
                                        <i class="fa-solid fa-user-slash text-xs"></i>
                                    </button>
                                </form>
                                <?php else: ?>
                                    <button type="button" class="act-btn act-btn-disabled" title="สถานะเต็มแล้ว">
                                        <i class="fa-solid fa-user-slash text-xs"></i>
                                    </button>
                                <?php endif; ?>
                                ...
                                <!-- Close Future Slots -->
                                <form method="POST" class="m-0"
                                    onsubmit="return confirm('ปิดรอบอนาคตทั้งหมดของแคมเปญนี้? ระบบจะไม่ยกเลิกผู้ที่จองไว้');">
                                    <?php csrf_field(); ?>
                                    <input type="hidden" name="action" value="close_future_slots">
                                    <input type="hidden" name="campaign_id" value="<?= $c['id'] ?>">
                                    <button type="submit" class="act-btn act-btn-slots" title="ปิดรอบทั้งหมด">
                                ...
                                <?php else: ?>
                                    <button type="button" class="act-btn act-btn-disabled" title="มีผู้ลงทะเบียนแล้ว">
                                        <i class="fa-solid fa-trash-can text-xs"></i>
                                    </button>
                                <?php endif; ?>
                            </div>
