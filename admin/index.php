...
/* ──────────────────────────────────────────────────────────────────────
   Satisfaction Survey (formerly on KPI page · merged back per user req)
   ────────────────────────────────────────────────────────────────────── */
try {
...
$sv_comments = []; $sv_total_rows = 0; $sv_page = 1; $sv_pages = 1;
...
if (!function_exists('dash_star_html')) {
    function dash_star_html(float $avg, string $size = 'text-sm'): string {
        $html = '';
        for ($i = 1; $i <= 5; $i++) {
...
if (!function_exists('dash_week_trend')) {
    function dash_week_trend(int $curr, int $prev): string {
...
if (!function_exists('dash_pager_url')) {
    function dash_pager_url(string $param, int $page): string {
...
/* ── Satisfaction Survey cards (merged from kpi.php) ───────────── */
.sv-section-label {
    display: flex; align-items: center; gap: 8px;
    margin-bottom: 14px; margin-top: 32px;
}
.sv-section-label .dot {
    width: 12px; height: 12px; border-radius: 999px;
    background: #F59E0B;
    box-shadow: 0 0 6px rgba(245,158,11,.5);
}
.sv-section-label span {
...
.sv-card:hover { box-shadow: 0 8px 24px rgba(0,0,0,.1); transform: translateY(-2px); }
.sv-card-accent {
    position: absolute; right: 0; top: 0; width: 6px; height: 100%;
    border-radius: 0 18px 18px 0;
}
.sv-bar-track { background: #f1f5f9; border-radius: 99px; height: 8px; overflow: hidden; }
.sv-bar-fill  { height: 100%; border-radius: 99px; transition: width .6s cubic-bezier(.16,1,.3,1); }

body[data-theme='dark'] .sv-card {
    background: #1e293b; border-color: #334155;
    box-shadow: 0 1px 3px rgba(0,0,0,.3), 0 4px 16px rgba(0,0,0,.2);
}
body[data-theme='dark'] .sv-bar-track { background: #334155; }
body[data-theme='dark'] .sv-section-label span { color: #94a3b8; }
</style>
...
<div class="sv-section-label">
    <div class="dot"></div>
    <span>ผลการประเมิน (Satisfaction Survey)</span>
</div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-5">
    ...
    <!-- Avg rating -->
    <div class="sv-card p-4 sm:p-5">
        <div class="sv-card-accent" style="background:linear-gradient(180deg,#F59E0B,#D97706)"></div>
    ...
    <!-- Total responses -->
    <div class="sv-card p-4 sm:p-5">
        <div class="sv-card-accent" style="background:linear-gradient(180deg,#8B5CF6,#6D28D9)"></div>
    ...
    <!-- This week -->
    <div class="sv-card p-4 sm:p-5">
        <div class="sv-card-accent" style="background:linear-gradient(180deg,#2563EB,#1D4ED8)"></div>
        ...
        <?= dash_week_trend($sv['this_week'], $sv['last_week']) ?>
    </div>

    <!-- Satisfaction rate -->
    <div class="sv-card p-4 sm:p-5">
        <div class="sv-card-accent" style="background:linear-gradient(180deg,#2e9e63,#16a34a)"></div>
...
<!-- Star distribution + Recent comments -->
<div class="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-8">

    <!-- Star distribution (2/5) -->
    <div class="lg:col-span-2 sv-card p-5">
        ...
        <div class="flex items-center gap-3 mb-3 last:mb-0">
            ...
            <div class="flex-1 sv-bar-track">
                <div class="sv-bar-fill" style="width:<?= $barW ?>%;background:<?= $color ?>"></div>
            </div>
    ...
    <!-- Recent comments (3/5) -->
    <div class="lg:col-span-3 sv-card p-5 flex flex-col">
                ...
                <?php foreach ($sv_comments as $c): ?>
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="py-2.5 pr-3">
                            <span class="flex gap-0.5"><?= dash_star_html((float)$c['rating'], 'text-[9px]') ?></span>
                        </td>
                        <td class="py-2.5 pr-3">
        ...
        <div class="mt-4 pt-3 border-t border-gray-100 flex items-center justify-center gap-1">
            ...
            ?>
            <a href="<?= dash_pager_url('cp', 1) ?>" class="px-2 py-1 text-xs rounded-lg <?= $sv_page===1?'pointer-events-none text-gray-300':'text-gray-500 hover:bg-gray-100' ?>">«</a>
            <a href="<?= dash_pager_url('cp', max(1,$sv_page-1)) ?>" class="px-2 py-1 text-xs rounded-lg <?= $sv_page===1?'pointer-events-none text-gray-300':'text-gray-500 hover:bg-gray-100' ?>">‹</a>
            <?php if ($show_first): ?>
                <a href="<?= dash_pager_url('cp', 1) ?>" class="px-2.5 py-1 text-xs rounded-lg text-gray-500 hover:bg-gray-100">1</a>
                <span class="text-gray-300 text-xs">…</span>
            <?php endif; ?>
            <?php for ($p = max(1,$sv_page-$window); $p <= min($sv_pages,$sv_page+$window); $p++): ?>
                <a href="<?= dash_pager_url('cp', $p) ?>"
            ...
            <?php if ($show_last): ?>
                <span class="text-gray-300 text-xs">…</span>
                <a href="<?= dash_pager_url('cp', $sv_pages) ?>" class="px-2.5 py-1 text-xs rounded-lg text-gray-500 hover:bg-gray-100"><?= $sv_pages ?></a>
            <?php endif; ?>
            <a href="<?= dash_pager_url('cp', min($sv_pages,$sv_page+1)) ?>" class="px-2 py-1 text-xs rounded-lg <?= $sv_page===$sv_pages?'pointer-events-none text-gray-300':'text-gray-500 hover:bg-gray-100' ?>">›</a>
            <a href="<?= dash_pager_url('cp', $sv_pages) ?>" class="px-2 py-1 text-xs rounded-lg <?= $sv_page===$sv_pages?'pointer-events-none text-gray-300':'text-gray-500 hover:bg-gray-100' ?>">»</a>
        </div>
        <?php endif; ?>
        <?php endif; ?>
    </div>
