...
Sidebar ใช้ `psb-section-label` (กลุ่ม) + `psb-item` (เมนู) — โครงสร้างแบ่งเป็น 9 sections **ห้ามเพิ่มเมนูลอยๆ** ทุกเมนูใหม่ต้องเข้ากลุ่มที่มีอยู่ หรือเพิ่ม section ใหม่ที่ตำแหน่งเหมาะสม
...
7. **ติดตามระบบ** (icon: `fa-binoculars`) — Activity Logs, Error Logs
8. *(spacer `<div style="flex:1"></div>`)*
9. **ข้อมูลหลัก** (icon: `fa-database`) — ข้อมูลคลินิก, นักศึกษาทุน, Master Data อื่นๆ
10. **ตั้งค่า** (icon: `fa-gear`) — Settings (อยู่ล่างสุดเสมอ)
...
3. **Section label format** เป็น collapsible toggle (accordion) — ต้องมี `data-group` key (`overview` / `ai` / `security` / `insurance` / `comm` / `inventory` / `monitor` / `masterdata` / `settings`) จับคู่ระหว่างปุ่มและ wrapper:
   ```html
