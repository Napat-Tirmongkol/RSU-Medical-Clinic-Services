...
Sidebar ใช้ `psb-section-toggle` (กลุ่ม) + `psb-item` (เมนู) — **โครงสร้าง 8 consolidated groups** (refactor พ.ค. 2569 จาก 15 groups เดิม) **ห้ามเพิ่มเมนูลอยๆ** ทุกเมนูใหม่ต้องเข้ากลุ่มที่มีอยู่ ใช้ `<div class="psb-divider">` แบ่งช่วงภายในกลุ่มเดียวกัน (เช่น insurance | pharmacy | safety ภายใน "งานคลินิก")

### ลำดับ section (จากบนลงล่าง)
1. **ภาพรวม** (data-group: `overview`, icon: `fa-chart-line`) — Dashboard, App Launcher, โปรไฟล์ของฉัน
2. **งานคลินิก** (data-group: `clinical`, icon: `fa-stethoscope`) — รวม 3 sub-areas (มี divider คั่น):
   - *Insurance & Gold Card*: Dashboard Workbook, Insurance Hub, ย้ายสิทธิ์บัตรทอง (+badge), บัตรทอง, สถิติบัตรทอง, อัพโหลดรายชื่อ, สถานะเอกสาร, Insurance Partners
   - *Pharmacy / Vaccine*: บันทึกการฉีดวัคซีน, ประเภทวัคซีน
   - *Safety*: บันทึกอุบัติเหตุ
3. **รายงาน & ตรวจสอบ** (data-group: `reports`, icon: `fa-clipboard-list`) — รวม:
   - *Reports*: สรุปงานประจำวัน, รายงานประจำเดือน, Productivity พยาบาล
   - *Monitor*: Activity Dashboard (superadmin), Activity Logs, Error Logs, Sentry Events, Database Schema, SQL Console
4. **ทรัพยากร** (data-group: `resources`, icon: `fa-cubes`) — รวม:
   - *Finance*: Cash Book, บริการคลินิก, บันทึกการเข้ารับบริการ, ใบแจ้งหนี้, ลูกหนี้คงค้าง, Payroll × 2
   - *Inventory*: ครุภัณฑ์สำนักงาน (ext link `/asset/`), วัสดุสิ้นเปลือง (ext link `/consumables/`)
5. **สื่อสาร & AI** (data-group: `comm`, icon: `fa-bullhorn`) — รวม:
   - *Comm*: ประกาศ, สารบรรณอิเล็กทรอนิกส์ (EDMS + sub-views: งาน/Tasks, SLA Dashboard, นโยบาย SLA)
   - *AI Suite*: AI Assistant, ผู้ช่วยข้อมูล, AI QA Lab, AI Prompts, AI Knowledge (gate `access_ai`)
   - *LINE Official*: LINE Chat (gate `access_ai` เช่นกัน)
6. **ระบบ & สิทธิ์** (data-group: `system`, icon: `fa-shield-halved`) — รวม:
   - *Security*: Identity & Governance, ISO Governance (superadmin), PDPA Audit
   - *Documents Library*: คลังเอกสาร (proposal, ISO, รายงาน, คู่มือ) — gate `superadmin || admin`
7. *(spacer `<div style="flex:1"></div>`)*
8. **ข้อมูลหลัก** (data-group: `masterdata`, icon: `fa-database`) — ข้อมูลคลินิก, สมุดความดัน, นักศึกษาทุน, ตารางเวรพยาบาล
9. **ตั้งค่า** (data-group: `settings`, icon: `fa-gear`) — Settings (อยู่ล่างสุดเสมอ)

### กติกาเมื่อเพิ่มเมนูใหม่
1. **เลือกกลุ่มที่ตรงหน้าที่** — ห้ามใส่ใน "ภาพรวม" เป็น default
   - งานเกี่ยวกับผู้ป่วย/ประกัน/วัคซีน/อุบัติเหตุ → **งานคลินิก** (ใช้ `<div class="psb-divider">` ก่อนเมนูใหม่ถ้าอยู่คนละ sub-area)
   - รายงานสถิติ / log / monitor → **รายงาน & ตรวจสอบ**
   - การเงิน / บิลลิ่ง / สต็อก / ครุภัณฑ์ → **ทรัพยากร**
   - ประกาศ / AI / LINE / สารบรรณ → **สื่อสาร & AI**
   - สิทธิ์ / audit / ISO / เอกสารคู่มือ → **ระบบ & สิทธิ์**
   - master data ของคลินิก/บุคลากร → **ข้อมูลหลัก**
   - การตั้งค่าระบบทั้งคลินิก → **ตั้งค่า**
2. **ถ้าไม่เข้ากลุ่มไหน** → ทบทวนก่อนว่าเข้าได้จริงหรือไม่ (มัก fit ได้สักกลุ่ม) ก่อนสร้าง section ใหม่ — ถ้าจำเป็นต้องเพิ่ม section ใหม่ต้อง justify ใน PR description ว่าทำไมไม่ fit
3. **Section label format** เป็น collapsible toggle (accordion) — ต้องมี `data-group` key (`overview` / `clinical` / `reports` / `resources` / `comm` / `system` / `masterdata` / `settings`) จับคู่ระหว่างปุ่มและ wrapper:
   ```html
   ...
   - หลัง refactor พ.ค. 2569 — per-group collapse state **ไม่ persist** ข้าม navigation (clean ทุกครั้งโหลด); active group เปิดเสมอ + `overview` เปิดเสมอ; nav อื่นๆ ถูก collapse
   - ตอน sidebar ย่อ icon-only — toggle จะถูกซ่อน, items ใน group แสดงเสมอ (ไม่ขึ้นกับ collapse state)
   - **ภายใน group เดียวกัน** ใช้ `<div class="psb-divider"></div>` คั่น sub-area เช่น insurance / pharmacy / safety ภายใน "งานคลินิก" (style อยู่ใน `portal.css` — gradient เส้นบางๆ, ซ่อนตอน sidebar collapsed)
...
7. **Gate group header เอง** ก่อน render section toggle — ถ้า user มองไม่เห็น item ใด ๆ ใน group นั้น (เพราะ access flag ไม่ครบ) ให้ซ่อน toggle button ด้วย (empty group toggle สับสน user คลิกเข้าไปไม่เจออะไร)

หลังเพิ่มเมนูใหม่ — รีวิวว่า sidebar ยังอ่านง่าย ไม่ยาวจนต้อง scroll เกินจอ desktop · ปัจจุบัน "งานคลินิก" มีถึง 11 items (max ที่ยอมรับได้) → ถ้ามากกว่านี้ให้พิจารณา promote sub-area เป็น section ใหม่ หรือลดจำนวนเมนูในนั้น
