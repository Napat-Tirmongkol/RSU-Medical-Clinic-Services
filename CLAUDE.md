...
3. **Section label format** เป็น collapsible toggle (accordion) — ต้องมี `data-group` key (`overview` / `ai` / `security` / `insurance` / `comm` / `monitor` / `masterdata` / `settings`) จับคู่ระหว่างปุ่มและ wrapper:
   ```html
