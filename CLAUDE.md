...
Sidebar ใช้ `psb-section-label` (กลุ่ม) + `psb-item` (เมนู) — โครงสร้างแบ่งเป็น 10 sections **ห้ามเพิ่มเมนูลอยๆ** ทุกเมนูใหม่ต้องเข้ากลุ่มที่มีอยู่ หรือเพิ่ม section ใหม่ที่ตำแหน่งเหมาะสม
...
6. **คลังพัสดุ** (icon: `fa-warehouse`) — ครุภัณฑ์สำนักงาน (link ออกไป `/asset/`), วัสดุสิ้นเปลือง (link ออกไป `/consumables/`)
7. **การเงิน** (icon: `fa-money-bill-trend-up`) — Cash Book (รายรับ-รายจ่าย, หมวด, สรุป) — gate `superadmin || role=admin || access_finance`
8. **ติดตามระบบ** (icon: `fa-binoculars`) — Activity Dashboard (superadmin), Activity Logs, Error Logs
9. **รายงาน** (icon: `fa-clipboard-list`) — รายงานประจำเดือน — gate `superadmin || access_monthly_report || access_director_view`
10. **เอกสาร** (icon: `fa-folder-tree`, data-group: `docs`) — คลังเอกสาร (proposal, ISO, รายงาน, คู่มือ) — gate `superadmin || admin`
11. *(spacer `<div style="flex:1"></div>`)*
12. **ข้อมูลหลัก** (icon: `fa-database`) — ข้อมูลคลินิก, นักศึกษาทุน, Master Data อื่นๆ
13. **ตั้งค่า** (icon: `fa-gear`) — Settings (อยู่ล่างสุดเสมอ)
...
3. **Section label format** เป็น collapsible toggle (accordion) — ต้องมี `data-group` key (`overview` / `ai` / `security` / `insurance` / `comm` / `inventory` / `finance` / `monitor` / `masterdata` / `settings`) จับคู่ระหว่างปุ่มและ wrapper:
   ```html
